...
gui.Name = "Pilgrammedv4 Script by Georgiy/8"
...
-- Mining Tab Module с объединенным Ore ESP и Auto Mining
local function createMiningTab()
    ...
    -- Локальні змінні для ESP та Mining (общие)
    ...
    -- Общие функции для ESP и Mining
    ...
    -- Функція пошуку найближчої руди (учитывает selectedOres)
    ...
    -- Функція телепорту до руди (сбоку и смотрит на руду)
    local function teleportToOre(ore)
        ...
        local orePart = ore:FindFirstChildWhichIsA("BasePart", true)
        if orePart then
            local sideOffset = Vector3.new(3, 0, 0)  -- Сбоку по X (можно изменить)
    ...
    -- Функція ходьби до руди (для режима ходьбы, без поворота)
    ...
    -- Статус руд (общий для ESP и Mining)
    ...
    -- Функція відображення списку инструментов
    ...
    -- Автообновление ESP каждые 5 сек
    task.spawn(function()
        while wait(5) do
            if oreESPEnabled then
                updateOreESP()
